...
- Version of GMF format: {gmf_version}
...
def create_md_report(file_name, out_md_file):
    ...
    gmf_dict = parse_gmf_dict(gmf_dict, validate=True)

    header = HEADER_TEMPLATE.format(
        file_name=file_name,
        n_tables=len(gmf_dict["tables"]),
        gmf_version=gmf_dict["gmf_version"],
    ...
    md_str = header
    if "relations" in gmf_dict and len(gmf_dict["relations"]) > 0:
        md_str += "\n## Relations between tables\n\n"
        md_str += "\n".join(f"- {rel}" for rel in gmf_dict["relations"])
        md_str += "\n"
    for table in gmf_dict["tables"]:
        n_rows = table["n_rows"]
