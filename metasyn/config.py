...
from metasyn.varspec import VarDefaults, VarSpec, DistributionSpec
        ...
        if attr == "dist_spec":
            if self.data_free and self.var_spec.dist_spec.implements is None:
                if self.var_type not in self.meta_config.defaults.distribution:
                    raise ValueError(f"Please supply distribution for variable with name {self.name}"
                                      "that is datafree or set a default for the variable type"
                                     f"{self.var_type}")
